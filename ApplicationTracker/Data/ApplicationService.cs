    ...
    IDbContextFactory<ApplicationDbContext> factory)
{
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(x =>
                ...
                || (x.JobDescription != null
                    && x.JobDescription.ToLower().Contains(term))
                || (x.JobPostingUrl != null
                    && x.JobPostingUrl.ToLower().Contains(term))
                || (x.JobBoard != null
                    && x.JobBoard.ToLower().Contains(term)));
        }
    ...
    public async Task<DashboardSummary> GetSummaryAsync()
    {
        ...
        return new DashboardSummary(
            ...
            await applications.CountAsync(
                x => x.AppliedDate == today),
            await applications.CountAsync(
                x => x.AppliedDate >= weekStart),
            await applications.CountAsync(
                x => x.Status == ApplicationStatus.Screening
                    || x.Status == ApplicationStatus.Interview),
            await applications
                .GroupBy(x => x.AppliedDate)
                .OrderByDescending(x => x.Key)
                .Take(14)
                .Select(x => new DailyApplicationCount(
                    x.Key,
                    x.Count()))
                .ToListAsync());
    }
...
public record DailyApplicationCount(
    DateOnly Date,
    int Count);